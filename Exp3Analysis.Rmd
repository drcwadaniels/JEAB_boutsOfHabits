...
ggplot() + 
  geom_point(data=tbt_matrices_wfit, 
             aes(x=StartTimeOfResponse,y=log(IRT),color=as.factor(reCode_Session))) + 
  geom_line(data=tbt_matrices_wfit, 
            aes(x=StartTimeOfResponse,y=log(exp_total_short),color=as.factor(reCode_Session)),size=1.5,linetype="solid")+
  geom_line(data=tbt_matrices_wfit, 
            aes(x=StartTimeOfResponse,y=log(exp_total_long),color=as.factor(reCode_Session)),size=1.5,linetype="dashed")+
  facet_wrap(~reCode_Subject+reCode_Session)
...
ggplot() + 
  geom_point(data=long_quantil_df, 
             aes(x=log10(IRT),y=log10(quantiles),color=as.factor(reCode_Session))) + 
    geom_line(data=long_quantil_df, 
            aes(x=log10(IRT),y=log10(biexp_fit),color=as.factor(reCode_Session)),size=1)+
  facet_wrap(~reCode_Subject+reCode_Session, scales="free")

ggplot() + 
  geom_point(data=long_quantil_df, 
             aes(x=IRT,y=log10(quantiles),color=as.factor(reCode_Session))) + 
  geom_line(data=long_quantil_df, 
            aes(x=IRT,y=log10(biexp_fit),color=as.factor(reCode_Session)),size=1)+
  xlim(0,30)+
  facet_wrap(~reCode_Subject+reCode_Session,scales="free")
